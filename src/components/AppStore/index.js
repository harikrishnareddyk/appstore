...
class AppStore extends Component {
  state = {list: appsList, searchEl: '', active: ''}

  search = event => {
    const {list, searchEl} = this.state
  ...
  sorting = tabId => {
    const filtered = appsList.filter(j => j.category === tabId)
    this.setState(preState => ({...preState, active: tabId, list: filtered}))
  }
  ...
  render() {
    ...
    const filtered = list.filter(i =>
      i.appName.toLowerCase().includes(searchEl.toLowerCase()),
    )
    return (
      <div>
        <h1>App Store</h1>
        ...
        <ul>
          {tabsList.map(each => (
            <Tab each={each} key={each.tabId} sorting={this.sorting} />
          ))}
        </ul>

        <ul>
          {filtered.map(item => (
            <Apps item={item} key={item.appId} />
          ))}
        </ul>
      </div>
    )
